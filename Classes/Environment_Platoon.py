...
class Environ:
    ...
    def add_new_platoon_by_number(self, number_vehicle, size_platoon):
        ...
        self.V2I_Shadowing = np.random.normal(0, 3, [len(self.vehicles)//self.size_platoon,len(self.vehicles)//self.size_platoon])
    ...
    def renew_channel(self, number_vehicle, size_platoon):
        ...
        n_platoons = len(self.vehicles) // size_platoon
        self.V2V_pathloss = np.zeros((len(self.vehicles), len(self.vehicles))) + 50 * np.identity(len(self.vehicles))
        self.V2I_pathloss = np.zeros((n_platoons,n_platoons))+ 50 * np.identity(n_platoons)

        self.V2V_channels_abs = np.zeros((len(self.vehicles), len(self.vehicles)))
        self.V2I_channels_abs = np.zeros((n_platoons ,n_platoons))
        ...
        delta_distance_platoons = np.asarray([self.vehicles[p*size_platoon].velocity * self.time_slow for p in range(n_platoons)])
        for i in range(n_platoons):
            for j in range(i + 1, n_platoons):
                self.V2I_Shadowing[j][i] = self.V2I_Shadowing[i][j]  = \
                    self.V2Ichannels.get_shadowing(delta_distance_platoons[i] + delta_distance_platoons[j], self.V2I_Shadowing[i][j])
               
                self.V2I_pathloss[i, j] = self.V2I_pathloss[j, i] = \
                    self.V2Ichannels.get_path_loss(self.vehicles[i*size_platoon].position, self.vehicles[j*size_platoon].position)

        self.V2I_channels_abs = self.V2I_pathloss + self.V2I_Shadowing

    def renew_channels_fastfading(self):

        """ Renew fast fading channel """# V2V快衰落：(n_veh, n_veh) → (n_veh, n_veh, n_RB)
    ...
    # V2I快衰落（车头间通信）：(n_platoons, n_platoons) → (n_platoons, n_platoons, n_RB)
        V2I_channels_with_fastfading = np.repeat(self.V2I_channels_abs[:, :,np.newaxis], self.n_RB, axis=2)
    ...
    def Compute_Performance_Reward_Train(self, platoons_actions):
        ...
        self.platoon_V2I_Interference = np.zeros((int(self.n_Veh / self.size_platoon),int(self.n_Veh / self.size_platoon)))  # V2I interferences
        self.platoon_V2I_Signal = np.zeros((int(self.n_Veh / self.size_platoon),int(self.n_Veh / self.size_platoon)))  # V2I signals
            ...
            for j in range(len(indexes)):
                for k in range(len(indexes)):
                    if indexes[j, 0] != indexes[k, 0] and platoon_decision[indexes[j, 0], 0] == 0: # platoon j has selected the inter-platoon communication
                        for l in range(int(self.n_Veh / self.size_platoon)):
                            if indexes[k, 0] !=l and indexes[j, 0] !=l:
                                    # 过滤自干扰：发送方k≠接收方l、通信发起方j≠接收方l
                                # if not self.active_links[indexes[k, 0]] and platoon_decision[indexes[k, 0], 0] == 1:
                                #     continue
                                self.platoon_V2I_Interference[indexes[j, 0],l] += \
                                    10 ** ((power_selection[indexes[k, 0], 0] - self.V2I_channels_with_fastfading[indexes[k, 0], l,i] +
        ...
        for i in range(self.n_RB):
            ...
            for j in range(len(indexes)):
                if platoon_decision[indexes[j, 0], 0] == 0:
                    for l in range(int(self.n_Veh / self.size_platoon)):
                        if indexes[j, 0] != l :
                            self.platoon_V2I_Signal[indexes[j, 0], l] += 10 ** ((power_selection[indexes[j, 0], 0] - self.V2I_channels_with_fastfading[indexes[j, 0],l,i] +
                                                            2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        ...
        self.interplatoon_rate = (V2I_Rate * self.time_fast * self.bandwidth).max(axis=1)
    ...
    def act_for_training(self, actions):
        ...
        for i in range(int(self.n_Veh / self.size_platoon)):
            # per_user_reward[i] = (1.98)*(1-(Demand[i]/self.V2V_demand_size))-platoon_AoI[i]/50
    ...
    def Compute_Interference(self, platoons_actions):
        ...
        # ------------ Compute Interference --------------------
        V2I_Interference_state = np.zeros([int(self.n_Veh / self.size_platoon), int(self.n_Veh / self.size_platoon)]) + self.sig2
            ...
            for j in range(len(indexes)):
                for k in range(len(indexes)):
                    if indexes[j, 0] != indexes[k, 0] and platoon_decision[indexes[j, 0], 0] == 0:
                        for l in range(n_platoon):
                            if indexes[j,0] != l and indexes[k,0] != l:
                        # if not self.active_links[indexes[k, 0]] and platoon_decision[indexes[k, 0], 0] == 1:
                        #     continue
                                V2I_Interference_state[indexes[j, 0],l] += \
                                    10 ** ((power_selection[indexes[k, 0], 0] - self.V2I_channels_with_fastfading[
                                        indexes[k, 0] ,l, i] + 2 * self.vehAntGain - self.vehNoiseFigure ) / 10)
        ...
        for i in range(int(self.n_Veh / self.size_platoon)):
            if platoon_decision[i, 0] == 0:
                self.Interference_all[i] = np.max(self.V2I_Interference_all[i,:])
            else:
